...
class OpenExcelThread(Thread):
    """Проверка класса рабочего потока."""
    def __init__(self, ):
        global frame
        """Запуск класса рабочего потока."""
        Thread.__init__(self)
        self.start()  # Начало потока

    def run(self):
        """Запуск потока (Открытие экселя) """
        # Данный код будет исполнятся в новом потоке.

        if frame.tc1.GetValue() == '':
            frame.console.WriteText('- Попытка открыть отчет... Нужно снала создать файл а, потом открывать его!\n')
        else:
            if frame.tc2.GetValue() == 'Report.xlsx (Чтобы изменить имя, нажми \"Ввести имя файла\")':

                # subprocess.Popen(f'Report.xlsx')
                os.system('Report.xlsx')
                wx.CallAfter(pub.sendMessage, "update", msg="Thread finished!")
            else:
                frame.console.WriteText(f"- Открыт файл: {frame.tc2.GetValue()}\n")
                os.system(f'{frame.tc2.GetValue()}')
                wx.CallAfter(pub.sendMessage, "update", msg="Thread finished!")
...
class MyFrame(wx.Frame):
    ...
    def open_file(self, event):
        OpenExcelThread()  # создание потока для открытия excel - файла
